...
mseOutputPath <- "J:/Desiree/Sardine/SardineMSEGit/debugExample"

# Operating Model - Research Model ----------------------------------------

# directory for OM SS code
OMmodelPath <- "J:/Desiree/Sardine/SardineMSEGit/OM_K"
...
EMmodelPath <- "J:/Desiree/Sardine/SardineMSEGit/EM_K"
# EM starter.ss file must indicate init values are to be pulled from control.ss file, not ss.par

out <- run_SSMSE(scen_name_vec = "customr", #"margComps_SardineHCR",# name of the scenario
                 out_dir_scen_vec = mseOutputPath, # directory in which to run the scenario
                 iter_vec = c(72), # run with 5 iterations for now
                 ...
                 custom_MS_source = "J:/Desiree/Sardine/SardineMSEGit/R/MS_sar_hcr.R", # file location of the MS function
